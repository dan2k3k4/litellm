...
run "disabled_by_default_leaves_the_service_untouched" {
  ...
  assert {
    condition     = [for c in google_cloud_run_v2_service.gateway.template[0].containers : c.name] == ["gateway"]
    error_message = "The gateway service must stay single-container unless spend_worker_enabled is set."
  }

  assert {
    condition = !anytrue([
      for e in google_cloud_run_v2_service.gateway.template[0].containers[0].env : startswith(e.name, "LITELLM_SPEND_WORKER_")
    ])
...
run "enabled_adds_a_sidecar_that_shares_the_gateway_transport" {
  ...
  assert {
    condition     = [for c in google_cloud_run_v2_service.gateway.template[0].containers : c.name] == ["gateway", "spend-worker"]
    error_message = "Enabling the sidecar must append a spend-worker container after the gateway container."
  }

  assert {
    condition = alltrue([
      for c in google_cloud_run_v2_service.gateway.template[0].containers : (
  ...
  assert {
    condition = (
      google_cloud_run_v2_service.gateway.template[0].containers[1].image == local.gateway_image &&
      google_cloud_run_v2_service.gateway.template[0].containers[1].command == tolist(["sh", "-c"]) &&
      endswith(google_cloud_run_v2_service.gateway.template[0].containers[1].args[0], " && exec python -m gateway.spend_worker") &&
      strcontains(google_cloud_run_v2_service.gateway.template[0].containers[1].args[0], "export DATABASE_URL=") &&
      strcontains(google_cloud_run_v2_service.gateway.template[0].containers[1].args[0], "REDIS_SSL_CA_CERTS") &&
      { for e in google_cloud_run_v2_service.gateway.template[0].containers[1].env : e.name => e.value }["LITELLM_JOB_ROLE"] == "spend_worker"
    )
  ...
  assert {
    condition = (
      length(google_cloud_run_v2_service.gateway.template[0].containers[1].ports) == 0 &&
      google_cloud_run_v2_service.gateway.template[0].containers[1].resources[0].limits.cpu == "500m" &&
      google_cloud_run_v2_service.gateway.template[0].containers[1].resources[0].limits.memory == "1Gi"
    )
  ...
  assert {
    condition = (
      { for e in google_cloud_run_v2_service.gateway.template[0].containers[1].env : e.name => e.value }["OPENAI_API_BASE"] == "https://example.invalid" &&
      contains([for e in google_cloud_run_v2_service.gateway.template[0].containers[1].env : e.name], "DATABASE_HOST") &&
      contains([for e in google_cloud_run_v2_service.gateway.template[0].containers[1].env : e.name], "REDIS_HOST") &&
      contains([for e in google_cloud_run_v2_service.gateway.template[0].containers[1].env : e.name if length(e.value_source) > 0], "LITELLM_MASTER_KEY") &&
      contains([for e in google_cloud_run_v2_service.gateway.template[0].containers[1].env : e.name if length(e.value_source) > 0], "DATABASE_PASSWORD") &&
      contains([for e in google_cloud_run_v2_service.gateway.template[0].containers[1].env : e.name if length(e.value_source) > 0], "OPENAI_API_KEY")
    )
