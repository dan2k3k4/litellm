...
def prepare_codex(
    ...
    *,
    ...
    trust on the command, which is stable across launches. A session flag replaces the whole
    `hooks.Stop` list, so when the user's own config already declares hooks the launch leaves
    them alone and skips ours rather than silently dropping theirs.
    """
